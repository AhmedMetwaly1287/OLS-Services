...
const ManageUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "http://localhost:6001/api/User/GetAll",
        {
          headers: {
      ...
      setError("Failed to communicate with User Service");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteUser = async (userId) => {
    try {
      const token = sessionStorage.getItem("token");
      await axios.delete(
        `http://localhost:6001/api/User/DeleteUser/${userId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleMakeLibrarian = async (userId) => {
    try {
      ...
      await axios.put(
        `http://localhost:6001/api/User/MakeLibrarian/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleApproveUser = async (userId) => {
    try {
      const token = sessionStorage.getItem("token");
      await axios.put(
        `http://localhost:6001/api/User/ApproveUser/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
